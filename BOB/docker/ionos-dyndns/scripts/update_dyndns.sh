...
while true; do
  ...
  if [ "$CURRENT_IP" = "$PREV_IP" ] && [ -n "$PREV_IP" ]; then
    ...
    # IONOS API aufrufen und Antwort prüfen
    HTTP_CODE=$(wget --server-response --spider "$UPDATE_URL" 2>&1 | grep "HTTP/" | tail -1 | awk '{print $2}')
    
    if [ "$HTTP_CODE" = "200" ]; then
      log_info "Update successful (HTTP 200). DNS propagation may take 1-15 minutes."
      write_status "ok" "$CURRENT_IP" "Update successful (HTTP 200)"
    elif [ -n "$HTTP_CODE" ]; then
      log_error "Update failed with HTTP $HTTP_CODE"
      write_status "error" "$CURRENT_IP" "Update failed (HTTP $HTTP_CODE)"
      sleep "$INTERVAL" &
      wait $!
      continue
    else
      log_error "Update failed - no response from IONOS API"
      write_status "error" "$CURRENT_IP" "Update failed - no response"
      sleep "$INTERVAL" &
      wait $!
      continue
    fi
    
    # CrowdSec Whitelist aktualisieren (nur bei erfolgreicher IP-Änderung)
    if [ "$HTTP_CODE" = "200" ]; then
      log_info "Updating CrowdSec whitelist..."
      if [ -x "/home/hajo/docker/crowdsec/scripts/update-whitelist.sh" ]; then
        sudo /home/hajo/docker/crowdsec/scripts/update-whitelist.sh >/dev/null 2>&1 || \
          log_warn "CrowdSec whitelist update failed (non-critical)"
      else
        log_debug "CrowdSec whitelist script not found or not executable"
      fi
    fi
  fi
